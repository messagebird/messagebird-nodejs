...
/**
 * Returns object is empty or not.
 *
 * @param {Object} obj
 * @return {Boolean}
 */
...
/**
 * Returns empty string if body is object and has no key inside otherwise stringify the object.
 *
 * @param {Object} obj
 * @return {Boolean}
 */
function stringifyBody(body) {
  if (typeof body === 'string') {
    return body;
  } else if (typeof body === 'object' && isEmptyObject(body)) {
    return '';
  }
...
function generate(req, signingKey) {
  ...
  var getBodyBuffer = function () {
    var stringifiedBody = stringifyBody(req.body);
    var bodyHash = crypto.createHash(MESSAGEBIRD_REQUEST_HASH).update(stringifiedBody).digest();
  ...
  payload = new Buffer.concat([getTimeAndQueryBuffer(req), getBodyBuffer(req)]);
