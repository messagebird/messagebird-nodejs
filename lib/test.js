...
const webhookSignatureJWT = require('./signaturev2');
  ...
  ...(() => {
    ...
    return references.map(function (ref) {
      return () => {
        ...
        let opts = new webhookSignatureJWT.VerifyOptions();
        ...
        webhookSignatureJWT.verify(
          ...
          doTest(null, 'webhookSignatureJWT: ' + ref.name, [
            [ref.reason, ref.valid === true]
          ]);
        })
          .catch((err) => {
            doTest(ref.valid === false ? null : err, 'webhookSignatureJWT: ' + ref.name, [
