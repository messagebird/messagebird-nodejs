...
const signatureWebhookJwt = require('./signatureWebhookJwt');
  ...
  ...(() => {
    ...
    return references.map(function (ref) {
      return () => {
        ...
        let opts = new signatureWebhookJwt.VerifyOptions();
        ...
        signatureWebhookJwt.verify(
          ...
          doTest(null, 'signatureWebhookJwt: ' + ref.name, [
            [ref.reason, ref.valid === true]
          ]);
        })
          .catch((err) => {
            doTest(ref.valid === false ? null : err, 'signatureWebhookJwt: ' + ref.name, [
